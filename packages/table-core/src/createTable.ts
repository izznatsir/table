...
>() => Table<
    Overwrite<TGenerics, { Row: TSubGenerics['Row']; ColumnMeta: TSubGenerics['ColumnMeta'] }>
>
...
export type Table<TGenerics extends PartialGenerics> = {
    ...
    createDataColumn: <TAccessor extends AccessorFn<TGenerics['Row']> | keyof TGenerics['Row']>(
        accessor: TAccessor,
        column: Overwrite<
            TAccessor extends (...args: any[]) => any
                ...
                ? // Accessor Key
                  Overwrite<
                      ColumnDef<Overwrite<TGenerics, { Value: TGenerics['Row'][TAccessor] }>>,
                      {
                          id?: string
                      }
                  >
...
type InitTable<TRender extends AnyRender> = {
    ...
    createTable: CreateTableFactory<Overwrite<PartialGenerics, { Render: TRender }>>
}

//

export function init<TRender extends AnyRender>(opts: { render: TRender }): InitTable<TRender> {
    return {
