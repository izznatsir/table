import { TableInstance, Row, RowModel, AnyGenerics, PartialGenerics } from '../types'
...
export function groupRowsFn<TGenerics extends AnyGenerics>(
    ...
    const existingGrouping = groupingState.filter((columnId) => instance.getColumn(columnId))
    ...
    const groupUpRecursively = (rows: Row<TGenerics>[], depth = 0, parentId: string) => {
...
function groupBy<TGenerics extends PartialGenerics>(rows: Row<TGenerics>[], columnId: string) {
