import { getValue, flattenBy, functionalUpdate, propGetter, memo, RequiredKeys } from './utils'
    ...
    const finalInstance: TableInstance<TGenerics> = {
        ...
        getDefaultColumn: memo(
            ...
            (defaultColumn) => {
                ...
                return {
                    header: (props: HeaderRenderProps<Header<TGenerics>>) => props.header.column.id,
                    footer: (props: HeaderRenderProps<Header<TGenerics>>) => props.header.column.id,
                    cell: ({ value = '' }: { value: any }): ReturnType<UseRenderer<TGenerics>> =>
        ...
        createColumn: (columnDef, depth: number, parent) => {
            ...
            column = features.reduce((obj, feature) => {
                return Object.assign(obj, (feature as any).createColumn?.(column, instance))
            }, column)
        ...
        createCell: (row, column, value) => {
            const cell: CoreCell<TGenerics> = {
                ...
                column,
                value,
                getCellProps: (userProps) => instance.getCellProps(row.id, column.id, userProps)!,
                renderCell: () =>
                    column.cell
        ...
        getCoreRowModel: memo(
            ...
            (
                ...
                const accessRow = (
                    ...
                    // Make the row
                    const row = instance.createRow(id, originalRow, rowIndex, depth, values)
                    ...
                    if (instance.options.getSubRows) {
                        const originalSubRows = instance.options.getSubRows(originalRow, rowIndex)
