...
export const Sorting = {
    getDefaultColumn: <TGenerics extends PartialGenerics>(): SortingColumnDef<TGenerics> => {
        return {
            sortType: 'auto',
        }
    },
    ...
    getInstance: <TGenerics extends PartialGenerics>(
        ...
        return {
            ...
            getColumnSortingFn: (columnId) => {
                ...
                return isFunction(column.sortType)
                    ...
                    : (userSortTypes as Record<string, any>)?.[column.sortType as string] ??
                      (sortTypes[column.sortType as BuiltInSortType] as SortingFn<TGenerics>)
            },
            ...
            getColumnIsSorted: (columnId) => {
                const columnSort = instance.getState().sorting?.find((d) => d.id === columnId)
