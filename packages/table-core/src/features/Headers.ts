    ...
    createRow: <TGenerics extends PartialGenerics>(
        ...
        return {
            ...
            getLeftVisibleCells: memo(
                () => [row._getAllVisibleCells(), instance.getState().columnPinning.left, ,],
                (allCells, left) => {
                    const cells = (left ?? [])
            ...
            getRightVisibleCells: memo(
                () => [row._getAllVisibleCells(), instance.getState().columnPinning.right],
                (allCells, right) => {
                    const cells = (right ?? [])
                        .map((columnId) => allCells.find((cell) => cell.columnId === columnId)!)
                        .filter(Boolean)
        ...
        return {
            createHeader: (
                ...
                let header: CoreHeader<TGenerics> = {
                    ...
                    getHeaderProps: (userProps) => instance.getHeaderProps(header.id, userProps)!,
                    getFooterProps: (userProps) => instance.getFooterProps(header.id, userProps)!,
                    renderHeader: () =>
                        column.header
            ...
            getHeaderGroups: memo(
                ...
                (allColumns, leafColumns, left, right) => {
                    const leftColumns = leafColumns.filter((column) => left?.includes(column.id))
                    const rightColumns = leafColumns.filter((column) => right?.includes(column.id))
...
export function buildHeaderGroups<TGenerics extends PartialGenerics>(
    ...
    const createHeaderGroup = (headersToGroup: Header<TGenerics>[], depth: number) => {
