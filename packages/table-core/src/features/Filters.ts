...
import { functionalUpdate, isFunction, makeStateUpdater, memo, Overwrite } from '../utils'
...
export const Filters = {
    getDefaultColumn: <TGenerics extends PartialGenerics>(): FiltersColumnDef<TGenerics> => {
        return {
            filterType: 'auto',
        }
    },
    ...
    getDefaultOptions: <TGenerics extends PartialGenerics>(
        ...
        return {
            ...
            getColumnCanGlobalFilterFn: (column) => {
                const value = instance.getCoreRowModel().flatRows[0]?.getAllCellsByColumnId()[
                    column.id
                ]?.value
        ...
        return {
            ...
            getColumnFilterFn: (columnId) => {
                ...
                return isFunction(column.filterType)
                    ...
                    : (userFilterTypes as Record<string, any>)?.[column.filterType as string] ??
                      (filterTypes[column.filterType as BuiltInFilterType] as FilterFn<TGenerics>)
            },

            getGlobalFilterFn: () => {
                const { filterTypes: userFilterTypes, globalFilterType } = instance.options

                return isFunction(globalFilterType)
                    ? globalFilterType
                    : globalFilterType === 'auto'
                    ? instance.getGlobalAutoFilterFn()
                    : (userFilterTypes as Record<string, any>)?.[globalFilterType as string] ??
                      (filterTypes[globalFilterType as BuiltInFilterType] as FilterFn<TGenerics>)
            },
            ...
            getColumnIsFiltered: (columnId) => instance.getColumnFilterIndex(columnId) > -1,

            getColumnFilterValue: (columnId) =>
                instance.getState().columnFilters?.find((d) => d.id === columnId)?.value,

            getColumnFilterIndex: (columnId) =>
                instance.getState().columnFilters?.findIndex((d) => d.id === columnId) ?? -1,
            ...
            setColumnFilterValue: (columnId, value) => {
                ...
                instance.setColumnFilters((old) => {
                    ...
                    //
                    if (
                        shouldAutoRemoveFilter(filterFn as FilterFn<TGenerics>, newFilter, column)
                    ) {
